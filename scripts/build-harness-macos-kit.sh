...
cp -p "$SYMPHONY_SOURCE" "$KIT_ROOT/scripts/bin/harness-symphony"
chmod 755 "$KIT_ROOT/scripts/bin/harness-symphony"
sha256_file "$KIT_ROOT/scripts/bin/harness-symphony" > "$KIT_ROOT/scripts/bin/harness-symphony.sha256"
cp -p "$ROOT_DIR/scripts/harness-symphony" "$STAGE_DIR/bin/harness-symphony"
chmod 755 "$STAGE_DIR/bin/harness-symphony"

WEB_DIST_SOURCE="$ROOT_DIR/crates/harness-symphony/web-ui/dist"
[ -f "$WEB_DIST_SOURCE/index.html" ] || \
  fail "Symphony web UI dist is missing; run npm run build in crates/harness-symphony/web-ui first"
mkdir -p "$KIT_ROOT/web-ui-dist"
cp -R "$WEB_DIST_SOURCE/." "$KIT_ROOT/web-ui-dist/"
